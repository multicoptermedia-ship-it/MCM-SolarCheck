...
def render_docx(model:InspectionReportModel,output_path:str|Path,*,rendered_evidence:tuple[RenderedEvidence,...]=())->Path:
    ...
    for index,item in enumerate(model.evidence,1):
        ...
        fields=(('Classification',item.classification),('Module',item.module_id),('Service location',item.service_location_status),('Review priority',item.priority_level),('Priority score',item.priority_score),('Priority basis',item.priority_reason),('Thermal frame',item.thermal_frame_id),('RGB frame',item.rgb_frame_id),('Raw sensor value',item.raw_value),('Raw delta from median',item.raw_delta_from_median),('Temperature [°C]',item.temperature_c),('Frame GPS latitude',item.latitude),('Frame GPS longitude',item.longitude),('Reviewer',item.reviewer),('Review note',item.review_note))
