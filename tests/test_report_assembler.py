...
def test_confirmed_and_unclear_findings_on_same_module_keep_distinct_evidence_and_single_module_counts(tmp_path):
    db=_db(tmp_path)
    db.save_modules("P1",(PVModule("M1","T1",((0,0),(1,0),(1,1)),0.9,"test"),))
    with db.connect() as sql:
        sql.execute("INSERT INTO thermal_frames(project_id,frame_id,source_file,thermal_source,metadata_json) VALUES('P1','T1','t.jpg','test','{}')")
    ...
    report=assemble_inspection_report(db,"P1","R1",datetime(2026,9,25,8,tzinfo=timezone.utc))
    assert report.conspicuous_modules==1
    assert report.manual_review_modules==1
    assert len(report.details)==2
    confirmed=[item for item in report.details if item.review_status=="confirmed"]
    unclear=[item for item in report.details if item.review_status=="unclear"]
    assert len(confirmed)==1 and confirmed[0].module_id=="M1"
    assert confirmed[0].finding_label is not None
    assert confirmed[0].manual_inspection_required is False
...
def test_released_report_blocks_unreviewed_finding_even_when_same_module_has_confirmed_evidence(tmp_path):
    db=_db(tmp_path)
    db.save_modules("P1",(PVModule("M1","T1",((0,0),(1,0),(1,1)),0.9,"test"),))
    with db.connect() as sql:
        sql.execute("INSERT INTO thermal_frames(project_id,frame_id,source_file,thermal_source,metadata_json) VALUES('P1','T1','t.jpg','test','{}')")
        sql.execute("INSERT INTO findings(project_id,finding_id,thermal_frame_id,pixel_x,pixel_y,finding_type,module_id,reviewer_status,metadata_json) VALUES('P1','F1','T1',1,2,'hotspot_candidate','M1','confirmed','{}')")
        sql.execute("INSERT INTO finding_reviews(project_id,finding_id,status,reviewer,reviewed_at_utc) VALUES('P1','F1','confirmed','Inspector','2026-09-25T08:00:00+00:00')")
        sql.execute("INSERT INTO findings(project_id,finding_id,thermal_frame_id,pixel_x,pixel_y,finding_type,module_id,reviewer_status,metadata_json) VALUES('P1','U1','T1',3,4,'open_circuit_candidate','M1','unreviewed','{}')")
    ...
    assert draft.details[0].review_status=="confirmed"
    with pytest.raises(ValueError,match="reviewed findings"):
...
def test_reviewed_report_may_preserve_unreviewed_findings_but_released_report_may_not(tmp_path):
    db=_db(tmp_path)
    db.save_modules("P1",(PVModule("M1","T1",((0,0),(1,0),(1,1)),0.9,"test"),))
    with db.connect() as sql:
        sql.execute("INSERT INTO thermal_frames(project_id,frame_id,source_file,thermal_source,metadata_json) VALUES('P1','T1','t.jpg','test','{}')")
        ...
        sql.execute("INSERT INTO finding_reviews(project_id,finding_id,status,reviewer,reviewed_at_utc) VALUES('P1','F1','confirmed','Inspector','2026-09-25T08:00:00+00:00')")
        sql.execute("INSERT INTO findings(project_id,finding_id,thermal_frame_id,pixel_x,pixel_y,finding_type,module_id,reviewer_status,metadata_json) VALUES('P1','U1','T1',3,4,'open_circuit_candidate','M1','unreviewed','{}')")
    reviewed=assemble_inspection_report(
        db,"P1","REV1",datetime(2026,9,25,8,tzinfo=timezone.utc),release_status="reviewed"
    )
    assert reviewed.release_status=="reviewed"
    assert reviewed.conspicuous_modules==1
    assert len(reviewed.details)==1
    assert reviewed.details[0].review_status=="confirmed"
    with pytest.raises(ValueError,match="reviewed findings"):
        assemble_inspection_report(
...
def test_generated_confirmed_asset_keeps_source_frame_identity(tmp_path):
    from PIL import Image
    db=_db(tmp_path)
    thermal=tmp_path/"thermal.png"
    Image.new("RGB",(100,100)).save(thermal)
    ...
    with db.connect() as sql:
        ...
        sql.execute("INSERT INTO finding_reviews(project_id,finding_id,status,reviewer,reviewed_at_utc) VALUES('P1','F1','confirmed','Inspector','2026-09-25T08:00:00+00:00')")
    detail=assemble_inspection_report(db,"P1","R1",datetime(2026,9,25,8,tzinfo=timezone.utc),asset_dir=tmp_path/"assets").details[0]
    assert detail.thermal_image is not None
    assert detail.thermal_image.source_frame_id=="T1"
